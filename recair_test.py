#!/usr/bin/env python
#
# Raspberry Pi Dual Stepper Motor motor daemon
# Author : Bob Rathbone
# $Id: recair_test.py,v 1.1 2013/12/17 09:21:11 bob Exp $
...
class MyDaemon(Daemon):
	...
	def run(self):
		...
		log.message('Motor daemon running pid ' + str(os.getpid()), log.INFO)
		...
		while running:
			...
			if count > 0:
				...
				delay = 10*60
	    		log.message("Delay "+ str(delay) + " count "+ str(count), log.INFO)
			time.sleep(delay)

		# Stop the motors
		motora.stop()
		motorb.stop()
		log.message('Motor daemon exiting', log.INFO)
		sys.exit()
